...
export function Dashboard() {
  ...
  async function loadTransactions() {
    ...
    const formattedTransactions: DataListProps[] = transactions.map((item: DataListProps) => {
      if (item.type === 'income') {
        entriesValues += Number(item.amount);
      } else {
        expensesValues += Number(item.amount);
      }
    ...
    const total = entriesValues - expensesValues;

    setHighlightData({
      entries: {
        amount: entriesValues.toLocaleString('pt-BR', {
          style: 'currency',
          currency: 'BRL'
        })
      },
      expenses: {
        amount: expensesValues.toLocaleString('pt-BR', {
          style: 'currency',
          currency: 'BRL'
        })
      },
